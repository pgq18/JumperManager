...
**可以先启动映射，再启动目标服务。** 目标端口暂时没有服务时，界面会显示“等待目标服务”，隧道仍保持运行。服务启动后即可直接使用，无需重启映射；点击“检查”可更新显示状态。来源监听端口与目标服务端口不必相同，例如 `设备 A:50052 → 设备 B:50051`。
...
- **检查**：查看 SSH、端口和目标连接是否正常。检查通过不等于应用本身的业务功能完全正常。
...
隧道已经建立，但目标服务目前无法连接。请在目标设备启动服务，并确认服务端口和目标地址正确。修改映射中的端口不会改变服务本身的监听端口。服务恢复后，新连接可直接通过现有隧道，点击“检查”更新状态。
