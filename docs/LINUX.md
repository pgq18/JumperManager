...
**管理器和远端设备都无需安装 Python。** 映射启停和端口检查通过 SSH 完成，不会向远端安装程序或写入检查脚本。Linux 进程统计使用系统自带的工具和 `/proc`；工具缺失、权限不足或系统信息不完整时，进程数量显示未知，不影响隧道启停。
...
- **远端没有 Python**：无需安装。映射启停和端口检查不依赖远端 Python，也不要求远端有可写目录。
- **目标进程统计需要哪些系统工具**：远端 Linux 需要可读取的 `/proc`、`sh`、`od`，以及支持 `-L` 和 `-c` 的 `stat`。目标服务地址填写主机名时，统计还会使用 `getent` 解析地址；填写 IP 地址时不需要它。这些只影响进程统计，不是建立映射的前提。
- **目标进程数量未知**：可能是上述工具缺失、进程权限不足，或容器环境无法提供完整信息。不会把未知误报成“没有进程在用”，也不会因此停止已建立的映射。可查看映射详情中的原因；不能根据端口可以连接就猜测有几个进程。
