...
class TrayController:
    ...
    def __init__(self, url: str, data_dir: Path,
        ...
        self._counts = (0, 0, 0, 0)  # running, degraded, error, total
    ...
    def _refresh_status(self):
        try:
            ...
            statuses = [mapping.get("status") for mapping in mappings if isinstance(mapping, Mapping)]
            counts = (statuses.count("running"), statuses.count("degraded"), statuses.count("error"), len(statuses))
            with self._lock:
                self._counts = counts
    ...
    def _status_text(self, _item=None):
        with self._lock:
            ...
            if not self._has_status:
                return "正在读取映射状态…"
            running, degraded, errors, total = self._counts
            return f"映射：{running} 运行中 / {total} 总数 · {degraded + errors} 需关注"
    ...
    def _update_display(self):
        with self._lock:
            ...
            if self._exit_requested.is_set():
                ...
                running, degraded, errors, total = self._counts
                title = f"JumperManager · 运行 {running}/{total} · 降级 {degraded} · 异常 {errors}"
